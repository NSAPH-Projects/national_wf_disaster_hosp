# Find percent of ZCTA exposed to the 100 worst fires 
...
exp <- read_csv(here(
  "local_data",
  "intermediate_data",
  "exposed_population_counts_worst_100_fires.csv"
...
exp$month <- as.character(seq.Date(from = as.Date("2000-01-01"), 
                           by = "month", 
                           length.out = 228)[exp$month])
exp$month <- substr(exp$month, start = 1, stop = 7)
